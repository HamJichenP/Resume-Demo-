...
const addNewItem = () => {
  ...
  // 簡單的 ID 生成邏輯
  const newId = editableAbout.value.bullets.length > 0
    ? Math.max(...editableAbout.value.bullets.map(b => b.id)) + 1
    : 1;
  ...
  <div class="admin-block about-me-admin">
    ...
    <div class="list-management-section">
      ...
      <div class="add-item-form">
        <input
          v-model="newItemText"
          placeholder="新增一個項目 (例如：Specialized in...)"
          class="input-text"
        />
      ...
      <ul class="bullet-list-admin">
        <li
          v-for="item in editableAbout.bullets"
          :key="item.id"
          class="bullet-item-admin"
        >
          ...
          <div class="item-actions">
            <button v-if="!item.isEditing" @click="item.isEditing = true" class="edit-btn" title="編輯">
...
<style scoped>
/* 變數：請確保您的 Vue 應用程式中定義了這些 CSS 變數 */
/* --name-color (主橘色) */
/* --title-color (粉色) */
/* --icon-box-bg (淺米色/淡色背景) */
/* --resume-card-bg (卡片/輸入框白色背景) */
/* --color-text (文字深色) */
...
.edit-btn { background-color: var(--icon-box-bg); color: var(--name-color); }
.save-btn { background-color: #4caf50; color: white; } /* Green */
.delete-btn { background-color: #f44336; color: white; } /* Red */
.add-btn { background-color: var(--name-color); color: white; }

.edit-btn .icon-symbol, .save-btn .icon-symbol,
.delete-btn .icon-symbol, .add-btn .icon-symbol {
    font-size: 1.25rem;
}
